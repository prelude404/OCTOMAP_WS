...
//点云地图的回调函数
void rcvPointCloudCallBack(const octomap_msgs::Octomap & octo_map)
{   
    ROS_INFO("I reci");
    if(_has_map ) return;
    ...
    for (octomap::OcTree::tree_iterator it = sub_octree->begin_tree(),end = sub_octree->end_tree(); it != end; ++it) {
...
int main(int argc, char** argv)
{
    ...
    _map_sub  = nh.subscribe( "octomap_full",1, rcvPointCloudCallBack );
